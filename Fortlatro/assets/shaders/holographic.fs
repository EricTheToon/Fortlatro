...
extern MY_HIGHP_OR_MEDIUMP vec2 holographic;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // Rainbow shimmer waves
	vec2 wave_part1 = uv_scaled_centered + 40.*vec2(sin(-t / 120.0), cos(-t / 85.0));
	vec2 wave_part2 = uv_scaled_centered + 45.*vec2(cos( t / 65.0),  sin( t / 95.0));
	vec2 wave_part3 = uv_scaled_centered + 35.*vec2(sin( t / 75.0), cos(-t / 110.0));

    float wave_field = (1.+ (
        sin(length(wave_part1) / 15.0) + cos(length(wave_part2) / 20.0) * sin(wave_part2.x / 12.0) +
        sin(length(wave_part3) / 18.0) * cos(wave_part3.y / 14.0) ))/2.;

    // Create diagonal scanning effect
    float scan_angle = uv_scaled_centered.x * 0.8 + uv_scaled_centered.y * 1.2;
    ...
    if (holo_intensity > 0.75) {
        tex.rgb = mix(tex.rgb, vec3(1.0, 1.0, 1.0), (holo_intensity - 0.75) * 0.4);
    }
