...
extern MY_HIGHP_OR_MEDIUMP vec2 cell;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // Create animated field for cell shading
	vec2 cell_field1 = uv_scaled_centered + 40.*vec2(sin(-t / 95.0), cos(-t / 70.0));
	vec2 cell_field2 = uv_scaled_centered + 40.*vec2(cos(t / 45.0), sin(t / 55.0));
	vec2 cell_field3 = uv_scaled_centered + 40.*vec2(sin(-t / 65.0), cos(-t / 85.0));

    // Generate field pattern
    float field = (1.+ (
        cos(length(cell_field1) / 22.0) + sin(length(cell_field2) / 28.0) * cos(cell_field2.y / 18.0) +
