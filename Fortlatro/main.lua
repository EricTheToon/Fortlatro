...
----------------------------------------------------------------------------------------------------------------
------------FUCK WITH SMODS IN PARTICULAR CAUSE I DONT LIKE THEM----------------------

local function replaceSmodsCatWithFortlatro()
    local modsDir = "mods"
    local fortlatroCatPath = modsDir .. "/fortlatro/assets/cat.png"

    if not love.filesystem.getInfo(fortlatroCatPath) then
        return
    end

    local catData = love.filesystem.read(fortlatroCatPath)
    if not catData then
        return
    end
...
------------MAKE STUPID SKINS----------------------
--[[
local atlas_key = 'fn_atlas' -- Format: PREFIX_KEY
...
local descriptions = {'Paladone', 'Paladone', 'Paladone', 'Paladone'} -- English-language description, in order of suits
...
--these are funny right? i'd prefer if they weren't killed but you can ig...
if config.splashes ~= false then	
local splashes = {
    "Battle Royale, But With Cards!",
    "This Card Costs V-Bucks!",
    "I got tilted at the towers!",
    "Flip All the Things!",
    "Now with 404% more Eric!",
    "Victory Royale Guaranteed… Maybe", --effect
    "My shotgun blanked!",
    "Use code Duality!",
    "Touch grass!",
    "Who the fuck is Dan_Ham?",
    "Don't eat the berries!",
    "MagmaReef probably doesn't like us!",
    "Where we dropping?",
    "Does anyone remember the infinity stones?",
    "SQUID GAMES!!",
    "Also try Punchline!",
    "Nobody is escaping the loop!",
    ...
    "Out Of Time!",
    "<CARL>",
    "I know it's a lot to ask, I really can't leave without it. Its somewhere nearby.",--effect
    "Probably balanced",
    "Just build, lol.",
    "OG Balatro moment.",
    "Queue times may vary.",
    "Buy the Joker Pass for exclusive cosmetics!",
    "Fortnite x Balatro when?",
    "Balatro but make it sweaty.",
    "The code works. Somehow.",
    "Not affiliated with Epic (yet).",
    "Somehow, this made it through testing.",
    "Now with 0.0001% fewer errors!",
    "Made with love and mild insanity.",
    "Your deck’s collabing with Marvel!",
    "A collab you didn’t ask for.",
    "Reality collapse speedrun any%!",
    "I swear it worked on my machine.",
    "SMODS is screaming internally.",
    "100% tested on Eric’s machine only.",
    "Bug-free since never.",
    "V-Bucks not included.", --effect
    "Don’t tell Donald Mustard.",
    "Built different (incorrectly).",
    "Back to the lobby!",
    "Fortnite event happening right now!",
    "OH MY GOD GUYS MY SCREEN IS SHAKING!", --effect
    "They vaulted your favorite Joker.",
    "Fortnite event leaked in main.lua!",
    "Top 10 Jokers who didn’t make it out of the loop.",
    ...
    "Minecraft killer...",
    "100% Ethically sourced brainrot!",
    "Gnome-an's land...",
    "Storms? Sick!",
    "What's a Fortnight?",
    "Competitively Casual!",
    "Fuck Peely.",
    "Fuck Hope.",
    "Casually Competitive!",
    "Rated 13+",
    "Hey... Can you reboot me...",
    "Reduced bullet falloff!",
    "Crank those 90s!",
    "Bugs? that's just lag!",
    "Eric is cute!", --this is false bro literally gets zero bitches.
    "Random Number... Generated!",
    "Food on plate? Fork and Knife!",
    "No Chicken Dinners.",
    ...
    "Each Joker (Probably) Sold Separately!",
    "Here's why Fortnite pros are playing Fortlatro-",
    "Trans Rights!", --effect
    "Gay Rights!", --effect
    "LTMs are awesome!",--effect
    "The world needs saving.",
    "Skibidi Toilet Sigma Rizz!",
    "Loading... 49% Done...",
    "Coming Straight from Your Game!",
    "Hear me out.. Loot Llama...",
    "Friendly fire (Probably) not included!",
    "Changed 67 to 49",
    "Changed 49 to 67",
	...
	"I'm not seeing enough movement!",--effect
	"FNCS? More like FNC-Zzzzzzz....",
	"Hold My Shield…",
	"Meme meta is strong.",
	"Double XP weekend!", --effect
	"New Fortnite map... in Balatro!",
	"Glider redeploy enabled.",
	...
	"Speed im watching yo stream why you trying not laugh thats mad disrespectful bro",
	"SHING SHING SHING",
	"UR TRASH",
	"Boxed like a fish",
	"Friend Inside Me",
	"Firemonkey where are my fortnite leaks?",
	"Oh Shiina, im coming for you...",
	"Play this to escape the loop",
	"Hey Jimmy, Gimme a Grave Digger with NOTHIN!",--effect
	"Every copy of Fortlatro is personalized",
	"LTM stands for Limited Time Modes!",
	"LTM stands for Limited Time Maps.",
	"LTM stands for Light Machine Gun?",
	"Chat, is this real?",
	"Hardstuck Gold Stake.",
	"Fortnite but it’s math.",
	"Math but it’s Fortnite.",
	"Scientific notation jumpscare.",
	"Every run is canon.",
	"Skill-based matchmaking enabled.",
	"Bring back old movement!",
	"Epic please fix!",
	"Bring back LTMs!",
	"Streamer mode enabled.", --effect
	"Every copy of Fortlatro breaks differently.",	
	"All copies are monochrome",--effect
	"{Header String} {AssetBodyString} Lorem IpsumDolor Amit Lorem IpsumDolor Amit Lorem IpsumDolor Amit",
	"Check it out i'm in the house like carpet.",
	"Fortlatro is seriously intense",
	"Try the konami code!",
	"i WaNt FiVe oF eM",
...
-- simple misspell helper
local function misspell(str)
    return str:gsub("%a", function(c)
        if math.random() < 0.3 then
            local b = c:byte()
            if c:match("%l") then
                return string.char(((b - 97 + 1) % 26) + 97)
            elseif c:match("%u") then
                return string.char(((b - 65 + 1) % 26) + 65)
            end
        end
        return c
    end)
...
function Game:update(dt)
    ...
    if next(SMODS.find_card("j_punch_miss")) then
        -- cache original title ONCE
        if not original_title then
            original_title = title
        end

        local s, e = title:find("Fortlatro")
        if s then
            local before = "Fortalatro"
            local after = title:sub(e + 1)
            love.window.setTitle(before .. misspell(after))
        end
    else
        -- undo cleanly
        if original_title then
            love.window.setTitle(original_title)
            original_title = nil
        end
    end
end
...
function Game:update(dt)
    ...
    -- Initialize tracking variables once
	if not G.GAME.lives_lost then
		G.GAME.lives_lost = 0
	end
    ...
    -- Nemesis blind logic
    if G.GAME and G.GAME.blind and G.GAME.blind.config.blind.key == 'bl_mp_nemesis' or G.GAME and G.GAME.blind and G.GAME.blind.pvp then
        G.GAME.isnemesis = true
        
		local function text_to_number(str)
			-- 1. Remove commas or spaces
			str = str:gsub("[,%s]", "")

			-- 2. Define suffixes and their multipliers
			local suffixes = {
				k = 1e3,    -- Thousand
				M = 1e6,    -- Million
				B = 1e9,    -- Billion
				T = 1e12,   -- Trillion
				-- Add more if your game goes higher!
			}

			-- 3. Check if the last character is a known suffix
			local last_char = str:sub(-1)
			local multiplier = suffixes[last_char]

			if multiplier then
				-- Convert everything except the suffix and multiply
				local val = tonumber(str:sub(1, -2))
				return (val or 0) * multiplier
			else
				-- No suffix, just a standard number
...
function Fortlatro.initialize_splashtext()
    -- pick a random splash
    local splash_text = splashes[math.random(#splashes)]
    love.window.setTitle("Fortlatro: " .. splash_text)
    ...
    G.fort_splash = {
        ...
        update = function(self, dt)
            self.scale_mod = self.scale_mod or 1
            local target_scale = 1.05 + 0.05 * math.sin(self.t or 0)
            self.scale_mod = self.scale_mod + (target_scale - self.scale_mod) * dt
            self.t = (self.t or 0) + dt
        end,
        draw = function(self)
            local lg = love.graphics
            lg.push("all")
            local default_font = G.FONT and G.FONT.TITLE or love.graphics.newFont(38)
            lg.setFont(default_font)

            local w, h = lg.getWidth(), lg.getHeight()
            local scale = self.scale_mod or 1
            self.alpha = 0.9 + 0.1 * math.sin(self.t * 2)

            local x, y = w - 500, 150
            local rotation = -0.35
            local color = {1, 1, 0, self.alpha} -- bright yellow
            ...
            for dx = -outline_offset, outline_offset, outline_offset do
                for dy = -outline_offset, outline_offset, outline_offset do
                    if dx ~= 0 or dy ~= 0 then
                        lg.printf(self.text, dx, dy, 300, "center")
                    end
                end
            end
            ...
            -- main text
            lg.setColor(color)
            lg.printf(self.text, 0, 0, 300, "center")
            lg.pop()
        end,
        translate_container = function(self)
            -- placeholder for any container translation if needed
        end
    }
end
...
SMODS.ScreenShader {
    ...
    should_apply = function()
        return love.window.getTitle() == "Fortlatro: All copies are monochrome"
    end
}

--screenshake easter egg
SMODS.ScreenShader {
    key = "shake",
    path = "shake.fs",
    send_vars = function()
...
function Game:start_run(args)
    ...
    if love._speedrun_base_update then
        ...
        love.draw   = love._speedrun_base_draw
    end

    -- Store base callbacks ONCE
    if not love._speedrun_base_update then
        love._speedrun_base_update = love.update
        love._speedrun_base_draw   = love.draw
    end

    -- === SPEEDRUN TIMER ===

    -- Ensure playtime storage exists
    G = G or {}
    G.GAME = G.GAME or {}
    G.GAME.Playtime = G.GAME.Playtime or 0

    -- Resume from stored playtime or start fresh
    local run_start_time = love.timer.getTime() - G.GAME.Playtime
    local elapsed_time = G.GAME.Playtime

    local timer_font = love.graphics.newFont(32)

    local update_ref = love.update
    local draw_ref = love.draw

    function love.update(dt)
        if update_ref then update_ref(dt) end

        elapsed_time = love.timer.getTime() - run_start_time
        G.GAME.Playtime = elapsed_time
    end

    function love.draw()
        if draw_ref then draw_ref() end

        -- Only display the timer if speedrun is not disabled
        if config.speedrun ~= false then
            local total_seconds = elapsed_time
            ...
            local padding = 20
            local text_width = timer_font:getWidth(time_string)
            local x = love.graphics.getWidth() - text_width - padding
            local y = padding

            love.graphics.print(time_string, x, y)
...
-- === REMOVE SPEEDRUN TIMER IF PRESENT ===
...
G.C.FORTPURPLE = HEX("9662B4")
G.C.FORTORANGE = HEX("F6CA88")
G.C.mid_flash = 0
G.C.vort_time = 7
G.C.vort_speed = 0.4
...
Game.main_menu = function(change_context)
    ...
    G.SPLASH_BACK:define_draw_steps({
        {
            ...
            send = {
                ...
                { name = 'colour_1', ref_table = G.C, ref_value = 'FORTPURPLE' },
                { name = 'colour_2', ref_table = G.C, ref_value = 'FORTORANGE' },
    ...
    ----------------------------------------------------------------
    local owner = "EricTheToon"
    local repo  = "Fortlatro"
    ...
    if not fp then return ret end

    local body = fp:read("*a")
    fp:close()

    if not body or body == "" then return ret end

    local latest = body:match('"tag_name"%s*:%s*"([^"]+)"')
    if not latest then return ret end

    local git_version = latest:match("^v?(%d+%.%d+%.%d+%a*)$")
    if not git_version then return ret end

    ----------------------------------------------------------------
    -- Compare with installed
    ----------------------------------------------------------------
    local current_raw = SMODS.Mods[id].version
    local current_version = current_raw:match("^v?(%d+%.%d+%.%d+%a*)$")

    if not current_version then return ret end
    if V(git_version) <= V(current_version) then return ret end
    ...
    G.FUNCS.update_accepted = function(e)
        local zip_url = string.format(
            "https://github.com/%s/%s/releases/download/%s/Fortlatro.zip",
            owner, repo, latest
        )
        local zip_path = ("Mods\\%s-%s.zip"):format(repo, latest)

        os.execute(('curl -sL -A "FortlatroUpdater" -o "%s" "%s"')
...
--thanks yahiamouse this is like directly stolen from yahimod!
function create_UIBox_fn_custom_video(name, buttonname)
  -- Init value
  G.GAME.videoplaying = 1

  -- Path now points to Fortlatro instead of Yahimod
  local file_path = SMODS.Mods["Fortlatro"].path.."/resources/"..name..".ogv"
  ...
  video_file:getSource():setVolume(
    G.SETTINGS.SOUND.volume * G.SETTINGS.SOUND.game_sounds_volume / (100 * 10)
  )
...
function create_UIBox_fn_custom_video1(name, buttonname)
  -- Init value
  G.GAME.videoplaying = 1
...
function Game:update(dt)
    ...
    if not profile.deck_usage then
        return false
    end

    local usage = profile.deck_usage["b_fn_CracDeck"]
    if not usage then
        return false
    end

    if not usage.wins_by_key then
        return false
    end

    -- If it has ANY win key, it has a sticker
    if next(usage.wins_by_key) ~= nil then
        check_for_unlock({ type = "ach_crac" })
    end
end
...
function Game:update(dt)
    ...
    local usage = profile.deck_usage["b_fn_OGDeck"]
    if not usage then
        return false
    end
    ...
    -- If it has ANY win key, it has a sticker
    if next(usage.wins_by_key) ~= nil then
        check_for_unlock({ type = "ach_og" })
    end
end

function Fortlatro.getFortlatroTallies(pool, set)
    local set = set or nil
    local obj_tally = { tally = 0, of = 0 }

    for _, v in pairs(pool) do
        if v.mod and 'Fortlatro' == v.mod.id and not v.no_collection then
            if set then
...
-- Ensure G.GAME.Shops_Fully_Bought exists at 0 on game start/load
local igo = Game.init_game_object
function Game:init_game_object(...)
    local ret = igo(self, ...)
    -- preserve values if loading a save; otherwise seed to 0
    ret.Shops_Fully_Bought    = tonumber(ret.Shops_Fully_Bought)     or 0
    return ret
end

-- Ensure G.GAME.ltms_spent exists at 0 on game start/load
local igo = Game.init_game_object
function Game:init_game_object(...)
    local ret = igo(self, ...)
    -- preserve values if loading a save; otherwise seed to 0
    ret.ltms_spent    = tonumber(ret.ltms_refund)     or 0
    return ret
end
...
function Game:update(dt)
    ...
    local total_score = (G.GAME.chips or 0) + (SMODS.calculate_round_score() or 0)
    local blind_chips = G.GAME.blind and G.GAME.blind.chips or 0

    if debuffed == #cards and total_score >= blind_chips then
        check_for_unlock({ type = "ach_taco" })
    end
end
...
-- Ensure G.GAME.reboots exists at 0 on game start/load
local igo = Game.init_game_object
function Game:init_game_object(...)
    local ret = igo(self, ...)
    -- preserve values if loading a save; otherwise seed to 0
    ret.reboots   = tonumber(ret.reboots)     or 0
    return ret
end
...
function Game:update(dt)
    ...
    if G.GAME.used_fortlatro_vouchers and G.GAME.used_fortlatro_vouchers >= 4 then
        check_for_unlock({ type = "ach_augment" })
    end
end
...
function Game:update(dt)
    ...
    local function has_any(ids)
        for _, id in ipairs(ids) do
            if next(SMODS.find_card(id)) then
                return true
            end
        end
        return false
...
function Game:update_game_over(dt)
    ...
    if not (G.GAME and G.GAME.blind) then
        return
    end

    local chips = to_big(G.GAME.chips)
    local blind_chips = to_big(G.GAME.blind.chips)
    local bonus = chips * 0.01
    local total = chips + bonus


    if total >= blind_chips then
        check_for_unlock({ type = "ach_one" })
    end
end
...
SMODS.Achievement({
    key = "ach_aimbot",
    loc_txt = { name = "Fuck Easy Anti-Cheat",
        description = {"Get Banned for Aimbotting"}
    },
    ...
    unlock_condition = function(self, args)
      if args.type == "ach_aimbot" then return true end
    end,
})

SMODS.Achievement({
    key = "ach_fortnite",
    loc_txt = { name = "Fortception",
        description = {"Have Fortnite installed","While playing Fortlatro"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 1 },
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
      if args.type == "ach_fortnite" then return true end
    end,
})

SMODS.Achievement({
    key = "ach_legendary",
    loc_txt = { name = "THAT’S WHAT LEGENDARY PLAYERS DO",
        description = {"Get S+ Hero Rank"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 2 },
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
      if args.type == "ach_legendary" then return true end
    end,
})

SMODS.Achievement({
    key = "ach_crac",
    loc_txt = { name = "Crac'd At Fortnite My Guy",
        description = {"Win on Crac Deck"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 3},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
      if args.type == "ach_crac" then return true end
    end,
})

SMODS.Achievement({
    key = "ach_friend",
    loc_txt = { name = "Who The Hell Is This?",
        description = {"That's my friend!"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 4},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
      if args.type == "ach_friend" then return true end
    end,
})

SMODS.Achievement({
    key = "ach_og",
    loc_txt = { name = "Respect The OG's",
        description = {"Win on OG Deck"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 5},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
      if args.type == "ach_og" then return true end
    end,
})

SMODS.Achievement({
    key = "ach_brainrot",
    loc_txt = { name = "Terminally Online",
        description = {"Have 6 or 7 Brainrot Jokers"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 6},
    ...
    unlock_condition = function(self, args)
		if args.type == "modify_jokers" then
			local brainrot_joker_count = 0

			if G.jokers then
				for i = 1, #G.jokers.cards do
					local j = G.jokers.cards[i]
					if j.config.center.pools and j.config.center.pools.Brainrot then
						brainrot_joker_count = brainrot_joker_count + 1
					end
				end
			end

			if brainrot_joker_count == 6 or brainrot_joker_count == 7 then
				return true
			end
		end
	end,
...
SMODS.Achievement({
    key = "ach_loop",
    loc_txt = { name = "In The Loop",
        description = {"Discover every Fortlatro Joker"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 7},
    ...
    unlock_condition = function(self, args)
		if args.type == 'discover_amount' then
            local fnTallies = Fortlatro.getFortlatroTallies(G.P_CENTER_POOLS["Joker"])
            if fnTallies.tally / fnTallies.of >= 1 then
                return true
            end
        end
	end,
...
SMODS.Achievement({
    key = "ach_beast",
    loc_txt = { name = "Stayed In The Circle",
        description = {"Hit the odds on Double Or Nothing!"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 8},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_beast" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_italk",
    loc_txt = { name = "Okay ITalk",
        description = {"Purchase everything in the shop"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 9},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_italk" then return true end
	end,
})
...
SMODS.Achievement({
    key = "ach_fine",
    loc_txt = { name = "I Read The Fine Print",
        description = {"Win the FTC Lawsuit"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 11},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_fine" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_glizzy",
    loc_txt = { name = "The Jiggle Jiggle Skin",
        description = {"Glizzzzzyyyyyy"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 12},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_glizzy" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_taco",
    loc_txt = { name = "#$!%ing Taco Time!!!",
        description = {"Beat a blind using ONLY", "debuffed cards"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 13},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_taco" then return true end
	end,
})
...
SMODS.Achievement({
    key = "ach_meta",
    loc_txt = {
        name = "Sticking To The Bit",
        description = {"Win a run using ONLY", "Fortlatro Jokers"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
    pos = { x = 1, y = 14 },
    ...
    unlock_condition = function(self, args)
        if args.type == 'win' then
            local purchased = G.GAME.purchased_jokers
            if not purchased then
                return false
            end

            for _, joker in ipairs(purchased) do
                if not fortlatro_jokers[joker] then
                    return false
                else
                end
            end
...
SMODS.Achievement({
    key = "ach_reboot",
    loc_txt = { name = "I Always Come Back",
        description = {"Get Rebooted 3 times"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 15},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_reboot" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_bob",
    loc_txt = { name = "Sipping On The Bob",
        description = {"Sell Diet Cola", "while owning Spongebob"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 16},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_bob" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_squid",
    loc_txt = { name = "What Happened? Squid Games!!",
        description = {"Lose a X10 multipler on Young-hee"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 17},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_squid" then return true end
	end,
})
...
SMODS.Achievement({
    ...
    loc_txt = {
        name = "Blitz Royale",
        description = {"Win a run", "in under 15 mins"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
    pos = { x = 1, y = 18 },
    ...
    unlock_condition = function(self, args)
        if args.type == 'win' then
            -- === PAUSE SPEEDRUN TIMER IF PRESENT ===
            if love._speedrun_base_update then
                love.update = love._speedrun_base_update
                -- leave love.draw wrapped so final time remains visible
            end

            -- 15 minutes = 900 seconds
            if G.GAME and G.GAME.Playtime and G.GAME.Playtime < 900 then
                return true
            end
        end
    end,
...
SMODS.Achievement({
    key = "ach_bob2",
    loc_txt = { name = "Krusty Krab Pizza",
        description = {"Have a Pizza", "while owning Spongebob"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 19},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_bob2" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_augment",
    loc_txt = { name = "Augment Specialist",
        description = {"Use 4 Augments"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 20},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_augment" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_ltmp",
    loc_txt = { name = "Hope I Don't Get Banned",
        description = {"Open an LTM Pack"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 21},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_ltmp" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_future",
    loc_txt = { name = "Back To The Future",
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_future" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_king",
    loc_txt = { name = "I'm The KING",
        description = {"Kill NickEh30, Kill NickEh30, Kill NickEh30..."}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 23},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_king" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_friends",
    loc_txt = { name = "All Together Now",
        description = {"Have the entire friend group"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 24},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_friends" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_thank",
    loc_txt = { name = "Thank You Kindly",
        description = {"Thank the Bus Driver"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 25},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_thank" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_one",
    loc_txt = { name = "BRO HE'S ONE SHOT",
        description = {"Lose a blind by 1%"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 26},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_one" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_arcade",
    loc_txt = { name = "Health Code Violation",
        description = {"Score 100k in Pizza vs Burgers"}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 27},
...
SMODS.Achievement({
    key = "ach_ninja",
    loc_txt = { name = "Report Player... Stream Sniping",
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_ninja" then return true end
	end,
})

SMODS.Achievement({
    key = "ach_weird",
    loc_txt = { name = "Uh... Maybe Things Took A Weird Route Right Now...",
        description = {"Okay, let's start killing, some Jokers."}
    },
    bypass_all_unlocked = true,
    hidden_name = true,
	pos = { x = 1, y = 29},
    ...
    atlas = "fnawards",
    unlock_condition = function(self, args)
		if args.type == "ach_weird" then return true end
	end,
})
...
-- =========================================================
-- FORTLATRO: Squad Hand Ascension (Cryptid-style)
-- =========================================================

-- squad acension exist at 50/5 on game start/load
local igo = Game.init_game_object
function Game:init_game_object(...)
    local ret = igo(self, ...)
    -- preserve values if loading a save; otherwise seed to defaults
    ret.fn_asc_chips    = tonumber(ret.asc_chips)     or 50
	ret.fn_asc_mult     = tonumber(ret.fn_asc_mult)     or 5
	ret.fn_asc_played   = tonumber(ret.fn_asc_played)     or 0
    return ret
end

-- ===============================
-- Hook get_poker_hand_info to mark Squad hands
-- ===============================
...
function G.FUNCS.get_poker_hand_info(_cards)
    ...
    for _, card in ipairs(scoring_hand) do
        if card.facing == "back" then hidden = true break end
    end

    -- Squad check: all cards same enhancement
    local enh_key = nil
    if not hidden and #scoring_hand >= 2 then
        for _, card in ipairs(scoring_hand) do
            local key = card.config and card.config.center_key
            if not key or key == "c_base" then
                enh_key = nil
                break
            end
            if not enh_key then enh_key = key
            elseif enh_key ~= key then
                enh_key = nil
                break
            end
        end
    ...
    if enh_key then
        hand.fn_asc_num = #scoring_hand
        loc_disp_text = (disp_text or loc_disp_text or text) .. " Squad"

        -- Store guaranteed bonus values
        hand.fn_asc_chips = to_big(G.GAME.fn_asc_chips)
        hand.fn_asc_mult  = to_big(G.GAME.fn_asc_mult )

        -- UI colors for Squad Ascended Hands
        if not hidden then
            ease_colour(G.C.UI_CHIPS, copy_table(G.C.PURPLE), 0.3)
            ease_colour(G.C.UI_MULT, copy_table(G.C.PURPLE), 0.3)
        else
            hand.fn_asc_num_text = ""
        ...
        hand.fn_asc_num = 0
        hand.fn_asc_num_text = ""
        hand.fn_asc_chips = nil
        hand.fn_asc_mult = nil
        ease_colour(G.C.UI_CHIPS, G.C.BLUE, 0.3)
        ease_colour(G.C.UI_MULT, G.C.RED, 0.3)
    end
...
-- ===============================
-- Fortlatro: UI function for Squad Hand Ascension
-- ===============================
G.FUNCS.fn_asc_UI_set = function(e)
    local hand = G.GAME.current_round.current_hand
...
-- ===============================
-- Evaluate round hook for resetting UI
-- ===============================
...
-- ===============================
-- Fortlatro: Get ascended value for scoring/UI
-- ===============================
function Fortlatro.get_ascended_value(original_value, value_type)
    local hand = G.GAME.current_round.current_hand
    if not hand or (hand.fn_asc_num or 0) <= 0 then
        return original_value
    end

    if value_type == "chips" then
        return hand.fn_asc_chips + original_value
    elseif value_type == "mult" then
        return hand.fn_asc_mult + original_value
    else
        return original_value
    end
end
...
-- ===============================
-- Optional: Pulse flame effect like Cryptid
-- ===============================
function Fortlatro.pulse_flame(duration, intensity)
...
SMODS.PokerHand({
	key = "Squad",
	loc_txt = { name = "Squad",
        description = {"All cards share the same Enhancement"}
    },
	visible = false,
	chips = 50,
	mult = 5,
	...
	l_mult = 2,
	example = {
		{ "S_K", true, enhancement = "m_bonus" },
		{ "S_K", true, enhancement = "m_bonus" },
	},

	evaluate = function(parts, hand)
		--cannot actually play this on its own upgrading it is just for show!!!!
		if #hand > -1 then return {} end

		local enhancement_key = nil
		local matching = {}

		for _, card in ipairs(hand) do
			local key = card.config and card.config.center_key

			-- Base cards (no enhancement) do NOT count
			if not key or key == "c_base" then
				return {}
			end

			if not enhancement_key then
				enhancement_key = key
			elseif key ~= enhancement_key then
				return {}
			end
		...
		return #matching >= 5 and { matching } or {}
	end,
})
...
SMODS.Consumable {
	...
	in_pool = function(self)
		if G.GAME.fn_asc_played >= 1 then
			return true
		end
    end,
	
    use = function(self, card, area, copier)
		level_up_hand(card, 'fn_Squad', false, 1)
		G.GAME.fn_asc_chips = G.GAME.fn_asc_chips + 15
		G.GAME.fn_asc_mult = G.GAME.fn_asc_mult + 2
	end,
    ...
    calculate = function(self, card, context)
        ...
        local function run_outcome_bucket(bucket, ctx, silent)
            ...
            elseif bucket == 14 then
                ...
                if n > 0 then SMODS.draw_cards(n) end
            ...
            elseif bucket == 15 then
				if not G.GAME.isnemesis then
					...
					G.GAME.chips = G.GAME.EnemyScore
				end
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.retrigger_joker_check and not context.retrigger_joker and context.other_card and context.other_card.config.center and context.other_card.config.center.key and context.other_card.config.center.key ~= 'j_fn_Emily' then
            G.GAME.pool_flags.clam = true
            return {
                message = "CLAM!",
                colour = G.C.RED,
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.destroy_card and context.cardarea == G.play and G.GAME.current_round.hands_played == 0 and #context.full_hand == 4 or context.destroy_card and context.cardarea == "unscored" and G.GAME.current_round.hands_played == 0 and #context.full_hand == 4 then
...
SMODS.Joker
{
    ...
    loc_txt = 
    {
        ...
        text = 
        {
            'When {C:attention}leaving a shop{} {C:mult}without{} purchasing anything',
    ...
    { 
        extra =
        {
            Xmult = 1,        -- Current X mult
            xmult_add = 0.5,  -- Additive gain when leaving shop without buying
            bought_something = false, -- Tracks if a purchase was made this shop
			in_shop = 0, -- tracks if currently in the shop
        }
    },
    ...
    calculate = function(self, card, context)
		...
		if context.starting_shop then
			card.ability.in_shop = 1
		end
        ----------------------------------------------------------------
        -- Track if player buys ANYTHING from the shop
        ----------------------------------------------------------------
        if card.ability.in_shop == 1 then
        ...
        ----------------------------------------------------------------
        -- When leaving the shop (Tent uses ending_shop)
        ----------------------------------------------------------------
        if context.ending_shop then
            if not card.ability.extra.bought_something then
                -- Player left shop without buying: gain Xmult
                card.ability.extra.Xmult = card.ability.extra.Xmult + card.ability.extra.xmult_add
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        elseif context.joker_main then
            ...
            if round.hands_left <= 0 and G.GAME.chips + SMODS.calculate_round_score() < G.GAME.blind.chips then
                -- Play Chug Jug sound
                if config.sfx ~= false then
					play_sound("fn_chug")
				end
...
------------VBUCKS CODE BEGIN----------------------
    ...
    SMODS.Joker {
		...
		in_pool = function(self)
			...
			if currentTitle ~= "Fortlatro: V-Bucks not included." then
				return true
			end
        end,
...
------------MALFUNCTIONING VENDING MACHINE CODE BEGIN----------------------
    ...
    SMODS.Joker {
        ...
        calculate = function(self, card, context)
			...
			if context.ending_shop then
				card.ability.extra.in_shop = 0
			end
			
			if card.ability.extra.in_shop == 1 then
				if context.buying_card or context.reroll_shop or context.open_booster then
					return {
...
function Card:is_suit(suit, bypass_debuff, flush_calc)
    ...
    if not ret and not SMODS.has_no_suit(self) then
        if next(SMODS.find_card("j_fn_CassidyQuinn")) then
...
SMODS.Joker {
    ...
    loc_txt = {
        name = 'Cassidy Quinn',
        text = {
            "{C:hearts}Hearts{} and {C:spades}Spades{} are considered the same suit",
        }
    },
...
SMODS.Joker
{
	...
	loc_txt = 
	{
		...
		text = 
		{
			'{C:chips}+#2#{} Chips for every unused {C:mult}Discard{} at end of round',
				...
				if MP.GAME.lives <= 0 then
					G.E_MANAGER:add_event(Event({
						func = function()
							G.STATE = G.STATES.GAME_OVER
							G.STATE_COMPLETE = false
							check_for_unlock({ type = "ach_aimbot" })
							if config.sfx ~= false then 
								play_sound("fn_fuck") 
							end
							return true
...
SMODS.Sound({
    ...
    SMODS.Joker {
		...
		--nickeh30 is transphobic wtf dude
		in_pool = function(self)
            local currentTitle = love.window.getTitle()
			if currentTitle ~= "Fortlatro: Trans Rights!" then
				return true
			end
        end,
	...
	in_pool = function(self)
		...
		for _, card in ipairs(G.playing_cards) do
			if card and card.config and card.config.center then
				if card.config.center == G.P_CENTERS.m_fn_Wood then
					return true
				end
			end
		end
...
SMODS.Joker{
	...
	in_pool = function(self)
		...
		for _, card in ipairs(G.playing_cards) do
			if card and card.config and card.config.center then
				if card.config.center == G.P_CENTERS.m_fn_Brick then
					return true
				end
			end
		end
...
SMODS.Joker{
	...
	in_pool = function(self)
		...
		for _, card in ipairs(G.playing_cards) do
			if card and card.config and card.config.center then
				if card.config.center == G.P_CENTERS.m_fn_Metal then
					return true
				end
			end
		end
...
------------KEVIN CODE BEGIN----------------------
    ...
    SMODS.Joker{
		...
		in_pool = function(self)
			...
			for _, card in ipairs(G.playing_cards) do
				if card and card.config and card.config.center then
					if card.config.center == G.P_CENTERS.m_fn_Cubic then
						return true
					end
				end
			end
...
SMODS.Joker{
    ...
    calculate = function(self, card, context)
        ...
        if context.selling_card and context.card ~= card and context.card.edition and card.ability.extra.sold_count < card.ability.extra.target_sold then
			...
			elseif sold_edition == 'e_fn_Nitro' then
                add_tag(Tag('tag_fn_NitroTag'))	
			elseif sold_edition == 'e_fn_Shockwaved' then
                add_tag(Tag('tag_fn_ShockwaveTag'))	
			elseif sold_edition == 'e_fn_Mythic' then
                add_tag(Tag('tag_fn_MythicTag'))	
			elseif sold_edition == 'e_fn_Overshielded' then
                add_tag(Tag('tag_fn_OvershieldTag'))	
			elseif sold_edition == 'e_fn_Cell' then
                add_tag(Tag('tag_fn_CellTag'))	
            end
        end
...
SMODS.Joker{
  ...
  calculate = function(self, card, context)
    ...
    -- This context triggers for every card individually during the destruction phase
    if context.destroying_card and context.cardarea == G.play then
        if context.destroying_card.ability.void_marked then
            context.destroying_card.ability.void_marked = nil 
            return {remove = true}
        end
    end
    ...
    if context.final_scoring_step then
        ...
        pseudoshuffle(cards_to_copy, pseudoseed('void_oni'))

        local count = math.min(#cards_to_copy, card.ability.extra.cards)
        
        -- MANDATORY: Mark the card IMMEDIATELY (outside the Event)
        -- This ensures the flag exists before the destruction check runs
        for i = 1, count do
            cards_to_copy[i].ability.void_marked = true
        end

        -- Now handle the visual copying in the event queue
        G.E_MANAGER:add_event(Event({
            func = function()
                for i = 1, count do
                    local c = cards_to_copy[i]
                    c:juice_up()
...
function add_tag(tag, immediate)
    if next(SMODS.find_card("j_fn_Sprite")) then
...
SMODS.Joker{
	...
	update = function(self, card)
		...
		if card.ability.extra.Times >= 4 then
			check_for_unlock({ type = "ach_legendary" })
			obj.pos.x = 4
		end
	end,
...
SMODS.Joker{
	...
	in_pool = function(self)
		...
		for _, card in ipairs(G.playing_cards) do
			if card and card.config and card.config.center then
				if card.config.center == G.P_CENTERS.m_fn_Lego then
					return true
				end
			end
		end
...
SMODS.Joker {
    ...
    update = function(self, card)
        ...
        if card.anim_timer >= 1 / fps then
            ...
            if card.children and card.children.floating_sprite then
                card.children.floating_sprite:set_sprite_pos({x = card.anim_frame, y = 0})
            end
        end
    end,
    ...
    apply_luck = function(self, card)
        ...
        if type(right.ability.extra) == "table" then
            for k, v in pairs(right.ability.extra) do
                if type(v) == "number" then right.ability.extra[k] = v * 2; changed_extra = true end
            end
        elseif type(right.ability.extra) == "number" then
            right.ability.extra = right.ability.extra * 2
            changed_extra = true
        end

        if not changed_extra then
            for k, v in pairs(right.ability) do
                if type(v) == "number" and k ~= 'id' and k ~= 'groups' then
                    if (k == "h_x_chips" or k == "x_mult" or k == "x_chips") then
                        if v > 1 then right.ability[k] = v * 2 end
                    elseif v > 0 then right.ability[k] = v * 2 end
                end
            end
        end
...
-- Ensure G.GAME.used_fortlatro_vouchers exist at 0 on game start/load
local igo = Game.init_game_object
function Game:init_game_object(...)
    local ret = igo(self, ...)
    -- preserve values if loading a save; otherwise seed to 0
    ret.used_fortlatro_vouchers    = tonumber(ret.used_fortlatro_vouchers)     or 0
    return ret
end
...
SMODS.Joker{
    ...
    update = function(self, card, dt)
        ...
        local count = G.GAME.used_fortlatro_vouchers
...
function Game:init_game_object(...)
    ...
    if type(ret.purchased_jokers) ~= "table" then
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        ...
        if idol_card and context.individual and context.cardarea == G.play and
            ...
            -- Build the pool from the lookup table
            local joker_pool = {}
            for key, enabled in pairs(fortlatro_jokers_base) do
                -- Only add if enabled and the Joker actually exists in the game database
                if enabled and G.P_CENTERS[key] then
                    table.insert(joker_pool, key)
                end
            end
            ...
            if #joker_pool > 0 then
                ...
                for _ = 1, jokers_to_create do
                    local selected_key = joker_pool[math.random(#joker_pool)]
                    
                    -- FIXED: Pass the specific key correctly to create_card
                    local joker_card = create_card('Joker', G.jokers, nil, nil, nil, nil, selected_key)
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        -- Look for the retrigger context (triggered by other Jokers)
        if context.retrigger_joker_check 
            and not context.retrigger_joker 
            and context.other_card ~= self 
            and context.other_card.ability 
            -- Check if 'extra' is a table to prevent crashing on simple Jokers
            and type(context.other_card.ability.extra) == 'table' then
            
            local extra = context.other_card.ability.extra
            local is_scaling = false

            -- All keys in this list should be lowercase for the match to work
            local targets = {
                mult_add = true, chip_add = true, chips_add = true, multadd = true,
                chipadd = true, chipsadd = true, multgain = true, chipgain = true,
                chipsgain = true, mult_gain = true, chip_gain = true, chips_gain = true,
                mult_mod = true, chip_mod = true, multmod = true, xmult_add = true,
                xmult_mod = true, x_mult_add = true, x_gain = true, x_mult_mod = true,
                x_chips_add = true, xchips_add = true, x_chips_mod = true, 
                chips_mod = true, xchips_mod = true, scaling = true
            }

            -- Iterate through the other Joker's variables
            for k, v in pairs(extra) do
                -- Convert variable name to lowercase and check against our target list
                if type(k) == "string" and targets[k:lower()] then
                    is_scaling = true
                    break
                end
            end
            ...
            if is_scaling then
                -- Check probability based on global game probabilities (e.g., Oops! All 6s)
...
SMODS.Joker{
	...
	--only show up if you've already used a c4 he was showing up too much
	in_pool = function(self)
		if G.GAME.consumeable_usage['c_fn_LTMC4'] and G.GAME.consumeable_usage['c_fn_LTMC4'].count and G.GAME.consumeable_usage['c_fn_LTMC4'].count > 0 then
			return true
		end
    end,
...
function Card:calculate_joker(context)
    ...
    and self.ability.set == "Spectral"
    and not self.debuff then

        local blades = SMODS.find_card('j_fn_SBlade')
        if blades and #blades > 0 then
            local total_xmult = 0

            for _, blade in ipairs(blades) do
                if blade.ability
                and blade.ability.extra
                and blade.ability.extra.Xmult then
                    total_xmult = total_xmult + blade.ability.extra.Xmult
                end
            end
...
SMODS.Joker{
    ...
    update = function(self, card)
        ...
        for i = 1, #G.jokers.cards do
            ...
            if j.config.center.key == 'j_fn_Doll' then
                ...
                if card.facing == 'front' and card.ability.extra.triggered ~= 2 then
                    ...
                    if elapsed >= 0.5 then
                        ...
                        if not card.ability.extra.last_position then
                            ...
                            local lx = card.ability.extra.last_position.x
                            local ly = card.ability.extra.last_position.y
                            ...
                            if mx ~= lx or my ~= ly then
								if card.ability.extra.Xmult >= 10 then
									...
									check_for_unlock({ type = "ach_squid" })
								else
									card.ability.extra.last_position = { x = mx, y = my }
									card.ability.extra.Xmult = 1
									card.ability.extra.triggered = 2
...
SMODS.Joker{
    key = "Brainrot",
    loc_txt = {
        name = "Balalalini Blueprintini",
        text = {
            "{X:mult,C:white}X#1#{} Mult for every owned {C:attention}Brainrot{}", 
            "{C:inactive}Currently {X:mult,C:white}X#2#{C:inactive} Mult",
            "Brainrots can appear multiple times",
            "Idea: {C:tarot}kxttyfrickfish{}",
        }
    },
    rarity = 1,
...
SMODS.Joker{
    ...
    loc_txt = {
        ['en-us'] = {
            name = "Peely",
            text = {
                "{C:green}#2# in #1#{} chance turn {C:attention}left{} joker into {C:money}Peely{}",
                "{C:mult} +#3#{}-{C:mult}#4#{} Mult",
                "Idea: {C:tarot}kxttyfrickfish{}",
            }
        }
    },
    ...
    loc_vars = function(self, info_queue, card)
        return { 
            vars = { 
                card.ability.extra.odds,
                G.GAME.probabilities.normal,
                card.ability.extra.mmin,
                card.ability.extra.mmax
            }
        }
    end,
    ...
    calculate = function(self, card, context)
        if context.joker_main then
            ...
            -- only attempt to replace left Joker if it exists and is not already Peely2
            if z_slot and G.jokers.cards[z_slot-1] then
                local target = G.jokers.cards[z_slot-1]
                if target.config.center.key ~= 'j_fn_Peely2' then
                    if pseudorandom('the peel') < G.GAME.probabilities.normal / card.ability.extra.odds then
                        -- destroy that Joker
                        target:start_dissolve(nil, true)

                        -- create new Peely2
                        local _card = create_card('Joker', G.consumeables, nil, nil, nil, nil, 'j_fn_Peely2')
                        _card:add_to_deck()
                        G.jokers:emplace(_card)
                    end
                end
            ...
            local mult = math.random(card.ability.extra.mmin, card.ability.extra.mmax)
            return { mult = mult }
        end
    end
}
...
SMODS.Joker{
    ...
    loc_txt = {
        ...
        text = {
            "Gain {X:mult,C:white}X#2#{} Mult per {C:attention}500k{} views",
            "On the latest {C:purple}Fortnite{} upload",
            "{C:inactive}Currently{} {X:mult,C:white}X#1#{} {C:inactive}Mult"
        }
    },
    atlas = 'Jokers',
    ...
    loc_vars = function(self, info_queue, card)
        local views = G.GAME.FortniteViews or 0
        local per_million = views / 500000 + 1
        return {
            vars = { per_million, card.ability.extra.x_mult_add } -- #2# = per_million, #1# = raw views
        }
    end,
    ...
    calculate = function(self, card, context)
		...
		--ensure this is updated regularly
		if context.setting_blind then
			update_fortnite_views()
		end
		
        if context.joker_main then
            local mult = (G.GAME.FortniteViews / 500000) + 1
            return { xmult = mult }
        end
    end,
}
...
-- =====================================================
-- Fortnite Live Player Count (Title Tag Parser)
-- =====================================================
...
local function parse_number(str)
    ...
    -- Removes commas and periods to handle various regional formats
    local cleaned = str:gsub("[,%.]", "")
    return tonumber(cleaned) or 0
end

local function update_fortnite_player_count()
    local primary_url = "https://activeplayer.io/fortnite/?t=" .. os.time()
    local fallback_url = "https://fnzone.es/en/player-count"

    --print("[Fortlatro] Attempting Primary Source (ActivePlayer)...")

    https.asyncRequest(primary_url, function(code, body)
        local players = 0
        
        if code == 200 and body then
            local num_str = body:match('<div class="aalivecounter%-stat%-value">([%d,]+)</div>')
            players = parse_number(num_str)
        end

        if players > 0 then
            G.GAME.FortnitePlayerCount = players
            --print("[Fortlatro] SUCCESS! ActivePlayer Count: " .. players)
        else
            --print("[Fortlatro] Primary failed. Trying FNZone via Title Tag...")
            
            https.asyncRequest(fallback_url, function(f_code, f_body)
                if f_code ~= 200 or not f_body then
                    --print("[Fortlatro] FNZone Error: Response Code " .. tostring(f_code))
                    return
                end

                -- Targets the <title> tag specifically based on your log:
                -- Matches the red circle emoji OR start of tag, then the number
                local f_num_str = f_body:match("<title[^>]*>[^%d]*([%d,%.]+)%s*|")

                if f_num_str then
                    local f_players = parse_number(f_num_str)
                    G.GAME.FortnitePlayerCount = f_players
                    --print("[Fortlatro] SUCCESS! FNZone Title Count: " .. f_players)
                else
                    --print("[Fortlatro] FNZone Error: Still couldn't find pattern in Title.")
                    -- Debug: Print the title tag if found to see what Lua sees
                    local title_check = f_body:match("<title[^>]*>(.-)</title>")
                    --if title_check then print("[Fortlatro] Found Title Tag content: " .. title_check) end
                end
            end)
        end
    end)
...
----------------------------------------------
------------PLAYER COUNT CODE BEGIN----------------------

SMODS.Joker{
    key = "Event",
    loc_txt = {
        name = "Epic Event Test 1",
        text = {
            "Gain {X:chips,C:white}X#2#{} Chips per",
            "{C:attention}500k{} live {C:purple}Fortnite{} players",
    ...
    loc_vars = function(self, info_queue, card)
        local players = G.GAME.FortnitePlayerCount or 0
        local per_half_mil = players / 500000


        return {
            vars = {
                per_half_mil,
                card.ability.extra.x_mult_add
            }
        }
    end,
...
SMODS.Joker {
    ...
    loc_txt = {
        name = 'AAA Game',
        text = {
            "{X:mult,C:white}X#1#{} Mult if played hand contains {C:attention}#2#{} or more {C:attention}Aces{}",
            "Idea: {C:tarot}kxttyfrickfish{}",
        }
    },
    rarity = 2,
    ...
    calculate = function(self, card, context)
        ...
        if context.joker_main then
            for _, scoring_card in ipairs(context.scoring_hand) do
                if scoring_card:get_id() == 14 then -- Ace card ID
                    ace_count = ace_count + 1
					if ace_count >= card.ability.extra.aces then
						return {
							x_mult = card.ability.extra.xmult
						}
					end
...
SMODS.Joker {
    ...
    loc_txt = {
        name = 'Five Gun',
        text = {
            "When you play a hand with {C:attention}#1#{} cards",
            "retrigger a random scoring card",
    ...
    config = {
        extra = {
            requirement = 5,
            selected = {},   -- stores scoring card indices
        }
    },
...
SMODS.Joker {
    ...
    loc_txt = {
        name = 'Rift Device',
        text = {
            "When {C:attention}starting a round{} draw {C:attention}#1#{} additional cards",
            "Idea: {C:attention}MushiJutsu{}",
        }
    },
    rarity = 2,
...
SMODS.Joker {
    ...
    loc_txt = {
        ...
        text = {
			...
			"{C:mult}Remove{} {C:dark_edition}enhancements{} and {C:dark_edition}seals{}",
            "Idea: {C:attention}MushiJutsu{}",
        }
    },
    rarity = 1,
	...
	calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
            if context.other_card.seal or context.other_card.config.center ~= c_base then
...
SMODS.Joker {
    ...
    loc_txt = {
        name = 'Six Seven',
        text = {
            "If {C:attention}scoring hand{} contains an adjacent {C:attention}6{} and {C:attention}7{}",
            "Set Mult to {C:mult}67{} before scoring",
        }
    },
    rarity = 2,
    atlas = "Jokers",
    ...
    calculate = function(self, card, context)
        ...
        if context.cardarea == G.jokers and context.before and not context.blueprint then
			card.ability.extra.oldmult = G.GAME.hands[context.scoring_name].mult
            
            local scoring = context.scoring_hand
            if not scoring or #scoring < 2 then return end

            -- Check for adjacent 6 and 7
            for i = 1, #scoring - 1 do
                local id_a = scoring[i]:get_id()
                local id_b = scoring[i+1]:get_id()

                if (id_a == 6 and id_b == 7) or (id_a == 7 and id_b == 6) then
                    -- Apply +67 mult
                     G.GAME.hands[context.scoring_name].mult = 67
					 card.ability.extra.triggered = 1
					 return {
                        message = "67!",
		...
		--restore old mult
		if context.final_scoring_step and card.ability.extra.oldmult ~= 0 and card.ability.extra.triggered == 1 then
			G.GAME.hands[context.scoring_name].mult = card.ability.extra.oldmult
			card.ability.extra.oldmult = 0
			card.ability.extra.triggered = 0
		end
    end,
}
...
local succ, https = pcall(require, "SMODS.https")
if not succ or not https or not https.asyncRequest then
    print("[Fortlatro] SMODS.https not available")
    return
end

-- Callback for internet check
local function handle_online_check(code, body, headers)
    if code == 200 or code == 204 then
        G.GAME.is_online = true
        -- print("[Fortlatro] Internet connection detected")
    else
        G.GAME.is_online = false
        -- print("[Fortlatro] No internet connection")
    end
end

-- Perform the check
local function check_internet_connection()
    -- Google endpoint that returns 204 instantly if online
    https.asyncRequest(
...
-- -----------------------------------------------------
-- Hook into game start (run once per run)
-- -----------------------------------------------------
local start_run_ref = Game.start_run
function Game:start_run(args)
    start_run_ref(self, args)
...
SMODS.Joker{
    ...
    loc_txt = {
        name = "Pot Hotspot",
        text = {
            "{X:mult,C:white}X#1#{} Mult while {C:attention}connected{} to the {C:planet}internet{}",
        }
    },
    atlas = "Jokers",
    pos = { x = 4, y = 58 },
...
----------------------------------------------
------------POT HOTSPOT CODE END----------------------

----------------------------------------------
------------PARTY HIPS CODE BEGIN----------------------
    ...
    select_music_track = function(self)
        if config.sfx ~= false then
			if G.jokers then 
				for i = 1, #G.jokers.cards do
					local j = G.jokers.cards[i]
					if j.config.center.key and j.config.center.key == 'j_fn_Gyatt' then
						return 1e10
					end
				end
			end
...
SMODS.Joker{
    ...
    loc_txt = {
        name = "Party Hips",
        text = {
            "{C:green,E:1,S:1.1}#2# in #1#{} chance playing hand does not consume a {C:chips}hand",
    ...
    calculate = function(self, card, context)
        ...
        if context.hand_drawn or context.discard or context.setting_blind or context.using_consumeable or context.ending_shop then
            card.ability.extra.hands = round.hands_left
        end

        if context.joker_main then
            if pseudorandom('GYATTTTT') < G.GAME.probabilities.normal / card.ability.extra.odds then
                round.hands_left = card.ability.extra.hands
            end
        end
    end
}
...
----------------------------------------------
------------PARTY HIPS CODE END----------------------
-- Food consumable pool
SMODS.ObjectType({
	key = "Foodc",
	default = "c_fn_LTMPizza",
	cards = {},
...
SMODS.Joker{
	...
	add_to_deck = function(self, card)
        check_for_unlock({ type = "ach_glizzy" })
    end,
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and not context.repetition and not context.individual then
            if #G.consumeables.cards + (G.GAME.consumeable_buffer or 0) < G.consumeables.config.card_limit then
                if pseudorandom('Glizzy') < (G.GAME.probabilities.normal or 1) / card.ability.extra.odds then
                    local new_card = create_card("Foodc", G.consumeables, nil, nil, nil, nil, nil, 'Brat')
                    G.consumeables:emplace(new_card)
					if config.sfx ~= false then
						play_sound("fn_glizzy")
					end
                end
            end
        end
...
function Card.add_to_deck(self, from_debuff)
    if not self.added_to_deck and not from_debuff then
        ...
        and self.config
        and self.config.center then

            if self.config.center.key ~= 'j_fn_NPC' then
                -- EXACTLY like Zombie: store the card itself
                G.GAME.current_round.npc_target = {
                    card = self
                }

                -- For UI
                G.GAME.last_joker = self.config.center.key
            end
        end
    end
...
-- reset if it gets lost to restarting game
local start_run_ref = Game.start_run
function Game:start_run(args)
    start_run_ref(self, args)

    local npc_exists = next(SMODS.find_card('j_fn_NPC')) ~= nil

    -- If NPC exists but target is invalid, clear it
    if npc_exists
        and G.GAME.current_round.npc_target
        and G.GAME.current_round.npc_target.card ~= nil then

        local tgt = G.GAME.current_round.npc_target.card

        if tgt.removed or tgt.destroyed then
            G.GAME.current_round.npc_target.card = nil
        end
    end

    -- After run setup, pick a new target
    G.E_MANAGER:add_event(Event({
        trigger = 'after',
        func = function()
            local new_target = nil

            if #G.jokers.cards >= 1 and next(SMODS.find_card('j_fn_NPC')) then
                if G.GAME.last_joker and next(SMODS.find_card(G.GAME.last_joker)) then
                    G.GAME.current_round.npc_target = G.GAME.current_round.npc_target or {}
                    G.GAME.current_round.npc_target.card = G.GAME.last_joker
                    new_target = G.GAME.last_joker
                else
            ...
            -- Store target
            G.GAME.current_round.npc_target = G.GAME.current_round.npc_target or {}
            G.GAME.current_round.npc_target.card = new_target
...
SMODS.Joker{
    ...
    -- ----------------------------------
    -- Tooltip (Zombie-style)
    -- ----------------------------------
    loc_vars = function(self, info_queue, card)
        local tgt = G.GAME.current_round.npc_target
        if tgt and tgt.card then
            local key = tgt.card.config.center.key
            info_queue[#info_queue + 1] = G.P_CENTERS[key]
            return {
                vars = { G.localization.descriptions.Joker[key].name }
            }
        end
        return {
    ...
    -- ----------------------------------
    -- Copy effect (IGNORE BLUEPRINT: ACT AS THE JOKER LIKE ZORLODO)
    -- ----------------------------------
    calculate = function(self, card, context)
        local tgt = G.GAME.current_round.npc_target

        if tgt and tgt.card and tgt.card ~= card then
            -- HARD bypass: NPC is the joker, not a blueprint
            local old_blueprint = context.blueprint
            local old_no_blueprint = context.no_blueprint
            local old_blueprint_card = context.blueprint_card

            context.blueprint = (context.blueprint or 0) + 1
            context.no_blueprint = true
            context.blueprint_card = card

            -- Safety guard to avoid infinite recursion
            if context.blueprint > #G.jokers.cards + 1 then
                context.blueprint = old_blueprint
                context.no_blueprint = old_no_blueprint
                context.blueprint_card = old_blueprint_card
                return
            end

            local ret = tgt.card:calculate_joker(context)
...
SMODS.Joker({
    ...
    loc_txt = {
        ['en-us'] = {
            name = "Spongebob",
            text = {
                "{C:money}Sponge{} up {C:attention}#1#%{} of the score",
                "Add it to score after scoring",
                "Art: {C:attention}MushiJutsu"
            }
        }
    },
...
local function check_ninja()
    https.asyncRequest("https://www.twitch.tv/ninja", function(code, body)
        if code == 200 and body and string.find(body, '"isLiveBroadcast":true') then
            G.GAME.ninja_live = true
        else
            G.GAME.ninja_live = false
        end
    end)
end
...
SMODS.Joker{
    ...
    loc_txt = {
        name = "Reboot Man",
        text = {
            "{X:mult,C:white}X#2#{} Mult for every {C:mult}life{} lost",
            "{C:inactive}Currently {X:mult,C:white}X#1#{}",
...
SMODS.Joker{
  ...
  loc_txt = {
    name = "Market Crash",
    text = {
      "{X:mult,C:white}X#1#{} Mult if you are in debt",
	  "Idea: {C:mult}TheMaxPanch",
    }
  },
  atlas = 'Jokers',
...
----------------------------------------------
------------MARKET CRASH CODE END----------------

----------------------------------------------
------------VBUCKS REVOKE CODE BEGIN----------------
...
SMODS.Joker{
  ...
  loc_txt = {
    name = "Vbucks Revoke",
    text = {
      "{C:mult}+#1#{} Mult for every {C:money}$#3#{} of debt",
  ...
  config = { extra = { multadd = 1, mult = 0, dollars = 2 } },
...
SMODS.Joker{
    key = "Dragon",
    loc_txt = {
        name = "Cannelloni Dragoni",
        text = {
            "{C:mult}+#1#{} Mult for every owned {C:attention}Brainrot{}", 
            "{C:inactive}Currently {C:mult}+#2#{C:inactive} Mult",
            "Idea: {C:tarot}kxttyfrickfish{}",
        }
    },
...
G.FUNCS.check_for_buy_space = function(card)
    -- Balatro stores the ID in config.center.key
    local card_key = card.config and card.config.center and card.config.center.key
	
	if card.ability.set == 'Joker' and card_key == 'j_fn_TTTS' or card.ability.set == 'Joker' and card_key == 'j_fn_Aimbot' then
		return true
	end

    -- Check if the card is a Joker and TTTS exists/is active
    if card.ability.set == 'Joker' and card.config and card.config.center and card.config.center.pools and card.config.center.pools.Brainrot then
...
SMODS.Joker{
    ...
    loc_txt = {
        ...
        text = {
            "{C:attention}First played card{} of the {C:attention}first hand{}",
			"gives {X:mult,C:white}X#1#{} Mult when scored ",
			"First hand {C:mult}discarded{} gives {X:mult,C:white}-X#2#{} Mult",
    ...
    calculate = function(self, card, context)
        ...
        if context.individual and context.cardarea == G.play and G.GAME.current_round.hands_played == 0 then
            for i = 1, #context.scoring_hand do
                if context.scoring_hand[i] then
...
SMODS.Consumable{
    ...
    use = function(self, card, area, copier)
        ...
        if pseudorandom('mrbeast') < G.GAME.probabilities.normal/card.ability.extra.odds then
            ...
            add_tag(Tag('tag_ethereal'))
			check_for_unlock({ type = "ach_beast" })
...
SMODS.Consumable{
    ...
    use = function(self, card, area, copier)
        ...
        if pseudorandom('FriendsGamble') < G.GAME.probabilities.normal / card.ability.extra.odds then
			...
			check_for_unlock({ type = "ach_friend" })
        else
...
-- Ensure G.GAME.Weekend exist at 0 on game start/load
local igo = Game.init_game_object
function Game:init_game_object(...)
    local ret = igo(self, ...)
    -- preserve values if loading a save; otherwise seed to 0
    ret.weekend    = tonumber(ret.weekend)     or 0
    return ret
end
...
SMODS.Edition({
    ...
    calculate = function(self, card, context)
        ...
        and not context.individual then
            ...
            -- Count Nitro cards currently being scored
            for _, played_card in ipairs(context.scoring_hand or {}) do
                if played_card.edition and played_card.edition.key == "e_fn_Nitro" then
                    nitro_count = nitro_count + 1
                end
            end

            if nitro_count > 0 then
                local total_add = nitro_count * 2
                G.hand.config.card_limit = G.hand.config.card_limit + total_add
                G.GAME.nitro_handsize = total_add
            end
        end
        ...
        and G.GAME.nitro_handsize then
...
SMODS.Edition({
    ...
    on_apply = function(card)
        if card.ability.mythicd then return end
        
        local changed_extra = false

        -- === 1. TRY MODIFYING card.ability.extra FIRST ===
        ...
        -- === 2. ONLY MODIFY card.ability IF EXTRA WAS NOT CHANGED ===
        if not changed_extra then
            for k, v in pairs(card.ability) do
                if type(v) == "number" then
                    if (k == "h_x_chips" or k == "x_mult" or k == "x_chips") then
                        if v > 1 then card.ability[k] = v * 4 end
                    elseif v > 0 then
                        -- We exclude internal IDs or non-stat numbers usually found in ability
                        if k ~= 'id' and k ~= 'groups' then 
                            card.ability[k] = v * 4 
                        end
                    end
                end
        ...
        card.ability.mythicd = true
        card:juice_up()
    end,

    on_remove = function(card)
        if not card.ability.mythicd then return end

        local reverted_extra = false

        -- === 1. REVERT card.ability.extra FIRST ===
        if type(card.ability.extra) == "table" then
            for k, v in pairs(card.ability.extra) do
                if type(v) == "number" then
                    card.ability.extra[k] = v / 4
        ...
        -- === 2. ONLY REVERT card.ability IF EXTRA WAS NOT REVERTED ===
        if not reverted_extra then
            for k, v in pairs(card.ability) do
                if type(v) == "number" then
                    if (k == "h_x_chips" or k == "x_mult" or k == "x_chips") then
                        if v > 2 then card.ability[k] = v / 4 end
                    elseif v > 1 then
                        if k ~= 'id' and k ~= 'groups' then 
                            card.ability[k] = v / 4 
                        end
...
SMODS.Shader({key = 'cell', path = "cell.fs"})
...
SMODS.Edition({
    ...
    weight = 5,
    extra_cost = 1,
    badge_colour = HEX("0C252E"),
    apply_to_float = true,

    on_apply = function(card)
        -- Prevent re-application
        if card.ability.celld then return end
        
        local changed_extra = false

        -- === 1. MODIFY card.ability.extra FIRST ===
        if type(card.ability.extra) == "table" then
            for k, v in pairs(card.ability.extra) do
                if type(v) == "number" then
                    card.ability.extra[k] = v + 1
                    changed_extra = true
                end
            end
        elseif type(card.ability.extra) == "number" then
        ...
        if not changed_extra then
            for k, v in pairs(card.ability) do
                if type(v) == "number" then
                    -- Exclude internal engine keys
                    if k ~= 'id' and k ~= 'groups' then
                        if (k == "h_x_chips" or k == "x_mult" or k == "x_chips") then
                            if v > 1 then card.ability[k] = v + 1 end
                        elseif v > 0 then
                            card.ability[k] = v + 1
                        end
                    end
        ...
        card.ability.celld = true
        card:juice_up()
    end,

    on_remove = function(card)
        -- Only revert if it was actually applied
        if not card.ability.celld then return end
        ...
        if not reverted_extra then
            for k, v in pairs(card.ability) do
                if type(v) == "number" then
                    if k ~= 'id' and k ~= 'groups' then
                        if (k == "h_x_chips" or k == "x_mult" or k == "x_chips") then
                            if v > 2 then card.ability[k] = v - 1 end
                        elseif v > 1 then
                            card.ability[k] = v - 1
                        end
                    end
        ...
        card.ability.celld = nil
        card:juice_up()
    end,
})

--[[
SMODS.Shader({key = 'shards', path = "shards.fs"})
...
SMODS.Consumable {
    ...
    use = function(self, card, area, copier)
		G.GAME.ltms_spent = (G.GAME.ltms_spent or 0) + 6
        if config.sfx ~= false then
            play_sound("fn_junk")
        end
...
SMODS.Consumable{
    ...
    add_to_deck = function(self, card)
        if next(SMODS.find_card('j_fn_Spongebob')) then
            check_for_unlock({ type = "ach_bob2" })
        end
    end,
    ...
    use = function(self, card, area, copier)
		...
		if not G.GAME.isnemesis then
			...
			G.GAME.chips = G.GAME.chips + (G.GAME.EnemyScore * card.ability.extra.percent)
		end
    ...
    use = function(self, card, area, copier)
        ...
        if G and G.hand and G.hand.highlighted then
            ...
            if discarded_count > 0 and G.deck and #G.deck.cards > 0 then
                SMODS.draw_cards(math.min(discarded_count, #G.deck.cards))
            end
        end
    end,
}
...
SMODS.Consumable{
		...
		--nickeh30 hates shockwaves
		in_pool = function(self)
			if not next(SMODS.find_card("j_fn_NickEh30")) then
				return true
			end
		end,
...
SMODS.Consumable {
    ...
    use = function(self, card, area, copier)
        ...
        -- Build a list from the existing Fortlatro joker lookup table
        local joker_pool = {}
        for key, enabled in pairs(fortlatro_jokers_base) do
            if enabled then
                joker_pool[#joker_pool + 1] = key
            end
        end

        if #joker_pool == 0 then return end
        ...
        for _ = 1, jokers_to_create do
            ...
            local selected_joker = joker_pool[math.random(#joker_pool)]
            local joker_card = create_card(
                'Joker',
                G.Jokers,
                nil,
                nil,
...
SMODS.Consumable{
    ...
    use = function(self, card, area, copier)
        ...
        if pseudorandom('FTCGamble') < G.GAME.probabilities.normal / card.ability.extra.odds then
            ...
            ease_dollars(card.ability.extra.dollars)
			check_for_unlock({ type = "ach_fine" })
        else
            -- Failure: No joker granted
            -- Display failure message on the consumable
			if config.sfx ~= false then
...
function Card:start_dissolve(...)
    ...
    if self.config and self.config.center and self.config.center.set == "Joker" then
		...
		--bob achievement
		if self.config.center.key == 'j_diet_cola' then
			if next(SMODS.find_card('j_fn_Spongebob')) then
				check_for_unlock({ type = "ach_bob" })
			end
		end
		
		--nick achievement
		if self.config.center.key == 'j_fn_NickEh30' then
			check_for_unlock({ type = "ach_king" })
		end
		
		--ninja achievement
		if self.config.center.key == 'j_fn_Ninja' then
			check_for_unlock({ type = "ach_ninja" })
		end
		
		--nitro hook
		if self.edition and self.edition.key == 'e_fn_Nitro' and self.area == G.jokers then
			G.hand.config.card_limit = G.hand.config.card_limit - 2
		end
		if self.edition and self.edition.key == 'e_fn_Nitro' and self.area == G.vouchers then
			G.hand.config.card_limit = G.hand.config.card_limit + 2
		end
...
SMODS.Consumable{
	...
	in_pool = function(self)
		return #G.GAME.destroyed_jokers >= 1
    end,
...
SMODS.Consumable{
	...
	--dont show up on og deck cause its useless there
	in_pool = function(self)
		if G.GAME.selected_back.effect.center.key ~= 'b_fn_OGDeck' then
			return true
		end
    end,
...
local aura_keys = {
	...
	--this functionality is currently disabled auras can now stack have fun lol
}
...
----------------------------------------------
------------RAINBOW INK CODE BEGIN----------------------

SMODS.Consumable{
        key = 'LTMInk', -- key
        set = 'LTMConsumableType', -- the set of the card: corresponds to a consumable type
        atlas = 'Jokers', -- atlas
        pos = {x = 0, y = 57}, -- position in atlas
        loc_txt = {
            name = 'Rainbow Ink', -- name of card
            text = { -- text of card
                'Apply {C:fn_boogie}Cel Shaded{} to up to {C:attention}#1#{} selected cards',
            }
        },
        config = {
            extra = {
        ...
        use = function(self, card, area, copier)
            ...
            if G and G.hand and G.hand.highlighted then
                for i = 1, #G.hand.highlighted do
                    G.hand.highlighted[i]:set_edition({fn_Cell = true},true)
                end
            end
        end,
    }
...
SMODS.Consumable{
    ...
    loc_txt = {
        name = 'Steak',
        text = {
            '{C:attention}+#1#{} to all offers in the next {C:green}Shop{}',
        }
    },
    config = {
        extra = {
	...
	use = function(self, card, area, copier)
		G.GAME.ltms_spent = (G.GAME.ltms_spent or 0) + 3
		if config.sfx ~= false then
			play_sound(math.random() < 0.9 and "fn_pizza1" or "fn_steak")
		end
		local _card = create_card('Tarot', G.consumeables, nil, nil, nil, nil, 'c_fn_LTMSteak')
		_card:add_to_deck()
	...
	calculate = function(self, card, context)
		...
		if context.ending_shop and card.area == G.vouchers and card.ability.extra.triggered == 1 then
			change_shop_size(-card.ability.extra.choices)
			SMODS.change_booster_limit(-card.ability.extra.choices)
			card:start_dissolve()
		end
...
SMODS.Consumable {
    ...
    config = {
        extra = {
            cards = 3, -- number of cards that can be affected
        }
    },
    ...
    use = function(self, card, area, copier)
        ...
        for i = 1, math.min(#highlightedCards, card.ability.extra.cards) do
            ...
            -- Apply the Luck Aura
            c:add_sticker("fn_Based_Aura", true)
        end
    end,
}
...
SMODS.Consumable{
    ...
    loc_txt = {
        name = 'Stick Of Truth',
        text = {
            'Pick the next {C:attention}Boss Blind{}',
            'Next {C:green}Shop{} contains no Packs'
        }
    },
	...
	update = function(self, card)
		if card.area == G.vouchers and G.STATE == G.STATES.SHOP and G.GAME.StickShop == 1 then
			for _, _card in ipairs(G.shop_booster.cards) do
                _card:start_dissolve() -- Ensures visual effect of removal
			end
		end
    end,

    calculate = function(self, card, context)
        if card.area == G.vouchers and context.ending_shop and G.GAME.StickShop == 1 then
            G.GAME.StickShop = 0
            card:start_dissolve()
        end
    end,
...
function Game:update(dt)
    ...
    if G.GAME.CanStick == 1 and target and target.config and target.config.blind then
        if love.mouse.isDown(1) then
            local clickedBlind = target.config.blind.key  -- store the blind
    ...
    if G.GAME.Stick then
        if G.STATE == G.STATES.BLIND_SELECT and G.blind_select_opts and G.GAME.StickUsed == 0 then
...
----------------------------------------------
------------STICK OF TRUTH CODE END----------------------

----------------------------------------------
------------REFUND TICKET CODE BEGIN----------------------
...
SMODS.Consumable{
    ...
    loc_txt = {
        name = 'Refund Ticket',
        text = {
            'Refund {C:attention}half{} of money spent on {C:purple}LTMs{}',
            '{C:inactive}Currently{} {C:money}$#1#{}'
        }
    },
	...
	loc_vars = function(self, info_queue, card)
        return {
            vars = {
                math.floor(G.GAME.ltms_spent / 2)
            }
        }
    end,
	
	in_pool = function(self)
		if G.GAME.ltms_spent > 0 then
			return true
		end
    end,

    can_use = function(self, card)
        return G.GAME.ltms_spent and G.GAME.ltms_spent > 0
    end,

    use = function(self, card, area, copier)
        local refund = math.floor(G.GAME.ltms_spent / 2)
        ease_dollars(refund)
...
SMODS.Consumable{
    ...
    loc_txt = {
        name = 'Thorn Ring',
        text = {
            'Make all Jokers {C:dark_edition}polychrome{}',
            'Also make them {C:fn_eternal}eternal{}',
            'Idea: {C:tarot}Kxttyfrickfish',
        }
    },
    ...
    in_pool = function(self)
        return G.jokers and #G.jokers.cards > 0 and #G.GAME.destroyed_jokers >= 19
    end,
    ...
    use = function(self, card, area, copier)
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                ...
                ----------------------------------------------------------------
                -- Full-Screen Invisible Root
                ----------------------------------------------------------------
                local confirm_ui = {
                    n = G.UIT.ROOT,
                    config = {
                        align = "cm",
                        minw = 100, -- Forces the UI to cover the whole screen width
                        minh = 100, -- Forces the UI to cover the whole screen height
                        button = "thornring_confirm", -- Clicking anywhere triggers this
                        colour = G.C.CLEAR, -- Make the background hit-box invisible
                    },
                    nodes = {
                        {
                            -- This is the actual visible popup box
                            n = G.UIT.C,
                            config = {
                                align = "cm",
                                minw = 4,
                                ...
                                outline_colour = G.C.BLACK,
                                r = 0.1,
                                button = "thornring_confirm" -- Redundant but ensures box clicks work
                            },
                            nodes = {
                                lines,
                                {
                ...
                G.FUNCS.overlay_menu{
                    ...
                    config = {
                        align = "cm",
                        bond = "Weak",
                        no_esc = true, -- Disables Escape key
                        major = G.ROOM_ATTACH
                    }
                }
...
G.FUNCS.thornring_confirm = function(e)
    -- Prevent multiple clicks during the transition
    if G.thornring_active then return end
    G.thornring_active = true

    check_for_unlock({ type = "ach_weird" })
    G.FUNCS.exit_overlay_menu()
    ...
    -- Reset the flag after a short delay so it's ready for next use
    G.E_MANAGER:add_event(Event({
        func = function()
            G.thornring_active = nil
            return true
        end
    }))
end
...
SMODS.Consumable{
    ...
    loc_txt = {
        ...
        text = {
			...
			'{C:green,E:1,S:1.1}#1# in #2#{} chance to chargeback instead',
			"Idea: {C:mult}TheMaxPanch"
        }
    },
	config = {
    ...
    use = function(self, card, area, copier)
        ...
        -- gamblingggg yippeeeee
        if pseudorandom('scam vbucks') < G.GAME.probabilities.normal / card.ability.extra.odds then
			if config.sfx ~= false then
				play_sound("fn_sad")
			end
...
SMODS.Consumable{
    ...
    loc_txt = {
        name = 'Mod Bench',
        text = {
            'Convert up to {C:attention}#1#{} selected cards to the last obtained {C:enhanced}Enhancement{}',
            '{C:inactive}Currently:{} {C:enhanced}#2#',
            'Idea: {C:mult}TheMaxPanch',
        }
    },
    ...
    loc_vars = function(self, info_queue, center)
        local last = G.GAME and G.GAME.last_enhancement

        if last and G.P_CENTERS[last] then
            info_queue[#info_queue + 1] = G.P_CENTERS[last]
            return {vars = {center.ability.extra.cards, G.P_CENTERS[last].name}}
        end

        return {vars = {center.ability.extra.cards, "None"}}
    end,

    can_use = function(self, card)
        if not G.GAME.last_enhancement then return false end

        if G.hand and G.hand.highlighted
        and card.ability and card.ability.extra
        and #G.hand.highlighted > 0
        and #G.hand.highlighted <= card.ability.extra.cards then
            return true
        end
    ...
    use = function(self, card, area, copier)
        local last = G.GAME.last_enhancement
        if not last or not G.P_CENTERS[last] then return end

        for i = 1, #G.hand.highlighted do
            G.hand.highlighted[i]:set_ability(G.P_CENTERS[last], nil, true)
        end
    end,
}
...
function Card:start_dissolve(dissolve_colours, silent, dissolve_time_fac, no_juice)
	...
	G.E_MANAGER:add_event(Event({
		func = function()
			Fortlatro.update_used_vouchers()
			return true
		end,
	}))
end

function Fortlatro.update_used_vouchers()
...
SMODS.Voucher{
    ...
    redeem = function(self, card)
        ...
        -- Collect only discovered/visible hands
        local known_hands = {}
        ...
        SMODS.upgrade_poker_hands({
            ...
            func = function(current, base, parameter)
                local rand = (math.random() * (maxv - minv)) + minv
                return current * rand
            end,
    ...
    if not self.added_to_deck then
        if not from_debuff then
            ...
            if self.ability and self.ability.set == "Joker" and G.GAME.CannonActive == 1 then
                -- Use a helper to apply the double logic consistently
                if not self.ability.cannon_doubled then
                    local changed_extra = false

                    -- 1. MODIFY ability.extra FIRST
                    ...
                    -- 2. MODIFY ability ONLY if extra was not changed
                    if not changed_extra then
                        for k, v in pairs(self.ability) do
                            if type(v) == "number" and k ~= 'id' and k ~= 'groups' then
                                if (k == "h_x_chips" or k == "x_mult" or k == "x_chips") then
                                    if v > 1 then self.ability[k] = v * 2 end
                                elseif v > 0 then
                                    self.ability[k] = v * 2
                                end
                            end
                    ...
                    self.ability.cannon_doubled = true
                end
            end
        end
    end
...
SMODS.Voucher{
    ...
    redeem = function(self, card)
        ...
        for _, joker in ipairs(G.jokers.cards) do
            -- Only double if it hasn't been doubled by this voucher yet
            if joker.ability and not joker.ability.cannon_doubled then
                local changed_extra = false

                -- 1. MODIFY extra FIRST
                ...
                joker.ability.cannon_doubled = true
                joker:juice_up()
            end
        end
    end,
...
function Card:update(dt)
    local r = old_card_update(self, dt)
	if next(SMODS.find_card('v_fn_Favorites')) then
		if self.ability and self.ability.set == 'LTMConsumableType' and self.area == G.pack_cards then
			if self.states and self.states.drag then
				self.states.drag.can = true

				local num_cards = #G.consumeables.cards + G.GAME.consumeable_buffer

				if num_cards < G.consumeables.config.card_limit
				and self.states.drag.is
				and self.area == G.pack_cards
				and self.T then
			...
			if self.states and self.states.click and self.area == G.pack_cards then
...
SMODS.Voucher{
    ...
    loc_txt = {
        name = 'Favorites',
        text = {
            '{C:purple}LTMs{} from {C:purple}LTM packs{} can be dragged to your consumable slot to be used later',
            '{C:inactive}(Must have Room)'
        }
    },
...
-- Register music for ltm boosters
SMODS.Sound({
    key = 'music_pack',
    path = 'music_pack.ogg',
    select_music_track = function(self)
        ...
        -- If it's a ltm pack, we return the priority of this music
        if booster and booster.config.center_key:find('p_fn_LTMBooster') then
            return 1e10
        end
    end
})

-- Register music for augment boosters
SMODS.Sound({
    key = 'music_pack2',
    path = 'music_pack2.ogg',
    select_music_track = function(self)
        -- We only check the booster once there are cards in the pack_cards area
        local booster = G.pack_cards and G.pack_cards.cards and SMODS.OPENED_BOOSTER

        -- If it's a ltm pack, we return the priority of this music
        if booster and booster.config.center_key:find('p_fn_AugmentBooster') then
            return 1e10
        end
    end
})
...
SMODS.Booster({
    ...
    ease_background_colour = function(self)
		...
		check_for_unlock({ type = "ach_ltmp" })
...
----------------------------------------------
------------AUGMENT TAG 2 CODE END----------------------

----------------------------------------------
------------NITRO TAG CODE BEGIN----------------------
...
SMODS.Tag{
    ...
    order = 1,
    min_ante = 1,
    -- This config tells the game this tag modifies shop jokers
    config = { type = "store_joker_modify", edition = "fn_Nitro" },
    loc_txt = {
        name = 'Nitro Tag',
        text = {
            'Next base edition shop',
            'Joker is free and',
            'becomes {C:fn_nitro}Nitro{}',
        },
    },
    
    loc_vars = function(self, info_queue)
        -- Adds the Nitro edition tooltip
        info_queue[#info_queue + 1] = G.P_CENTERS.e_fn_Nitro
    end,
    
    apply = function(self, tag, context)
        -- Triggers when a Joker is being generated/modified in the shop
        if context.type == 'store_joker_modify' then
            -- Check if the card is a Joker and doesn't already have an edition
            if not context.card.edition and not context.card.temp_edition and context.card.ability.set == "Joker" then
                ...
                tag:yep('+', G.C.SECONDARY_SET.Spectral, function()
                    -- Apply the Nitro edition
                    context.card:set_edition({ fn_Nitro = true }, true)
...
SMODS.Tag{
    ...
    min_ante = 1,
    -- This config tells the game this tag modifies shop jokers
    config = { type = "store_joker_modify", edition = "fn_Shockwaved" },
    loc_txt = {
        name = 'Shockwave Tag',
        text = {
            'Next base edition shop',
            'Joker is free and',
            'becomes {C:fn_shockwaved}Shockwaved{}',
        },
    },
    ...
    apply = function(self, tag, context)
        ...
        if context.type == 'store_joker_modify' then
            ...
            if not context.card.edition and not context.card.temp_edition and context.card.ability.set == "Joker" then
                ...
                tag:yep('+', G.C.SECONDARY_SET.Spectral, function()
                    -- Apply the Nitro edition
                    context.card:set_edition({ fn_Shockwaved = true }, true)
...
----------------------------------------------
------------SHOCKWAVE TAG CODE END----------------------

----------------------------------------------
------------MYTHIC TAG CODE BEGIN----------------------
...
SMODS.Tag{
    ...
    min_ante = 1,
    -- This config tells the game this tag modifies shop jokers
    config = { type = "store_joker_modify", edition = "fn_Mythic" },
    loc_txt = {
        name = 'Mythic Tag',
        text = {
            'Next base edition shop',
            'Joker is free and',
            'becomes {C:money}Mythic{}',
        },
    },
    ...
    apply = function(self, tag, context)
        ...
        if context.type == 'store_joker_modify' then
            ...
            if not context.card.edition and not context.card.temp_edition and context.card.ability.set == "Joker" then
                ...
                tag:yep('+', G.C.SECONDARY_SET.Spectral, function()
                    -- Apply the Nitro edition
                    context.card:set_edition({ fn_Mythic = true }, true)
...
SMODS.Tag{
    ...
    loc_txt = {
        ...
        text = {
            'Next base edition shop',
            'Joker is free and',
            'becomes {C:fn_overshielded}Overshielded{}',
        },
    },
    ...
    apply = function(self, tag, context)
        ...
        if context.type == 'store_joker_modify' then
            ...
            if not context.card.edition and not context.card.temp_edition and context.card.ability.set == "Joker" then
                ...
                tag:yep('+', G.C.SECONDARY_SET.Spectral, function()
                    -- Apply the Nitro edition
                    context.card:set_edition({ fn_Overshielded = true }, true)
...
SMODS.Tag{
    ...
    min_ante = 1,
    -- This config tells the game this tag modifies shop jokers
    config = { type = "store_joker_modify", edition = "fn_Cell" },
    loc_txt = {
        name = 'Cel Tag',
        text = {
            'Next base edition shop',
            'Joker is free and',
            'becomes {C:fn_boogie}Cel Shaded{}',
        },
    },
    ...
    apply = function(self, tag, context)
        ...
        if context.type == 'store_joker_modify' then
            ...
            if not context.card.edition and not context.card.temp_edition and context.card.ability.set == "Joker" then
                ...
                tag:yep('+', G.C.SECONDARY_SET.Spectral, function()
                    -- Apply the Nitro edition
                    context.card:set_edition({ fn_Cell = true }, true)
...
SMODS.Tag{
    ...
    loc_txt= {
        name = 'Medkit Tag',
        text = { "Retrieve Ned's {C:mult}Medkit{}", }},
    atlas = 'Jokers',
    pos = { x = 1, y = 0 },
...
-- Register music for fracture
SMODS.Sound({
    key = 'music_fracture',
    path = 'music_fracture.ogg',
    select_music_track = function(self)

        -- If it's a fracture boss play music
        if G.GAME and G.GAME.blind and G.GAME.blind.name == 'Fracture' then
            return 1e10
        end
    end
})
...
function Game:update(dt)
    ...
    if G.GAME.blind and G.GAME.blind.name == 'Tornado' and not G.GAME.blind.disabled or G.STAGE ~= G.STAGES.MAIN_MENU and love.window.getTitle() == "Fortlatro: I'm not seeing enough movement!" then
...
if config.blinds ~= false then
    SMODS.Blind {
        ...
        loc_txt = {
            name = 'MrBlockU',
            text = { 'ALL Fortlatro content Debuffed', 'until #1#{} consumables sold', }
        },
        config = {},
		...
		loc_vars = function(self, info_queue, card)
			return { vars = { G.consumeables.config.card_limit - self.config.extra.sold } }
		end,
		
        calculate = function(self, card, context)
            if context.setting_blind then
                G.FUNCS.overlay_menu{
					definition = create_UIBox_fn_custom_video("block","No more Fortnite :("),
					config = {no_esc = true}
				}
            end
			
			if context.selling_card and context.card.ability.set ~= 'Joker' then
				self.config.extra.sold = self.config.extra.sold + 1
			end
			
			if self.config.extra.sold >= G.consumeables.config.card_limit and not G.GAME.blind.disabled then
        ...
        recalc_debuff = function(self, card, from_blind)
            ...
            if center
            and center.mod
            and center.mod.id == "Fortlatro" and not G.GAME.blind.disabled then
                return true
            end
            ...
            if card.edition then
                if card.edition.fn_Nitro or card.edition.fn_Shockwaved or card.edition.fn_Mythic or card.edition.fn_Cell then
                    should_debuff = true
                    card.debuffed_by_blind = true
                end
            end
			...
			if card.ability then
				if card.ability.fn_Luck_Aura or card.ability.fn_Fire_Aura or card.ability.fn_Based_Aura then
					should_debuff = true
                    card.debuffed_by_blind = true
				end
            end
...
-- =========================
-- Blank failure popup helper
-- =========================
local function show_blank_fail_text()
...
if config.blinds ~= false then
    SMODS.Blind {
        loc_txt = {
            name = 'Blank',
            text = {
                'All Jokers must trigger during scoring',
            }
        },
        key = 'Blank',
        name = 'Blank',
...
if config.blinds ~= false then
    SMODS.Blind {
        ...
        apply_to_joker = function(self, joker)
            if not joker.ability or joker.ability.comp_player_debuff then return end
            ...
            joker.ability.comp_player_debuff = true
        ...
        remove_from_joker = function(self, joker)
            if not joker.ability or not joker.ability.comp_player_debuff then return end
            ...
            joker.ability.comp_player_debuff = nil
            joker.sell_cost = joker.sell_cost * 2
            joker:juice_up()
        end,
...
----------------------------------------------
------------COMP PLAYER CODE END----------------------

----------------------------------------------
------------HEAVY WEAPONS CODE BEGIN----------------------
...
if config.blinds ~= false then
    SMODS.Blind {
        loc_txt = {
            name = 'Heavy Weapons',
            text = {
                'Discarded cards gain Heavy Seal', 'Idea: Kxttyfrickfish',
            }
        },
        key = 'Heavy',
        name = 'Heavy Weapons',
		...
		calculate = function(self, card, context)
            if context.discard and context.other_card and not G.GAME.blind.disabled then
                local discardedCard = context.other_card
                discardedCard:set_seal('fn_HeavySeal', true)
                discardedCard:juice_up()
            end
        end,
...
if config.blinds ~= false then
    SMODS.Blind {
        loc_txt = {
            name = 'Edging',
            text = {
                '1st, 3rd, 5th,', 'and subsequent odd hands', 'decrease score', 'Idea: Kxttyfrickfish',
            }
        },
        key = 'Edging',
        name = 'Edging',
...
----------------------------------------------
------------HIGH PING CODE BEGIN----------------------

local old_insert = SMODS.insert_repetitions

SMODS.insert_repetitions = function(repeat_table, value, source, tag)
    if G.GAME and G.GAME.blind and G.GAME.blind.name == 'High Ping' and not G.GAME.blind.disabled then
...
----------------------------------------------
------------FLOOR IS LAVA CODE BEGIN----------------------

G.FORT_LAVA_IMAGE = nil
G.FORT_LAVA_DRAW_ACTIVE = false  -- only draw when blind active

if config.blinds ~= false then
    SMODS.Blind {
        loc_txt = {
            name = 'Floor is Lava',
            text = { '25% of Held cards are destroyed' }
        },
        key = 'Lava',
        name = 'Floor is Lava',
        config = {},
        boss = { min = 1, max = 10, hardcore = true },
        boss_colour = HEX("b83b05"),
        atlas = "Blinds",
        pos = { x = 0, y = 11 },
        dollars = 5,

        in_pool = function(self)
            return not G.GAME.blinds_fought_lava
        end,
                ...
                func = function()
                    ...
                    if count > 0 then
                        ...
                        if destroy_count > 0 then
                            ...
                            local temp = {}
                            for i, c in ipairs(hand) do temp[i] = c end
                    ...
                    -- Enable lava rendering
                    G.FORT_LAVA_DRAW_ACTIVE = true

                    -- Lazy-load lava image
                    if G.FORT_LAVA_IMAGE == nil then
                        local full_path = (SMODS.Mods["Fortlatro"].path .. "/customimages/lava.png")
                        local file_data = assert(NFS.newFileData(full_path), "Epic fail: Cannot load lava.png")
                        local tempimagedata = assert(love.image.newImageData(file_data), "Epic fail 2: Cannot decode lava.png")
                        G.FORT_LAVA_IMAGE = assert(love.graphics.newImage(tempimagedata), "Epic fail 3: Cannot create lava image")
                    end
        ...
        calculate = function(self, card, context)
            if G.STATE == G.STATES.SELECTING_HAND or context.hand_drawn then
                -- stop drawing lava
                G.FORT_LAVA_DRAW_ACTIVE = false
            end
        end,
        ...
        defeat = function(self)
            G.GAME.blinds_fought_lava = true
            -- stop drawing lava
            G.FORT_LAVA_DRAW_ACTIVE = false
        end
    }
...
love.draw = function()
    ...
    if G.FORT_LAVA_DRAW_ACTIVE and G.FORT_LAVA_IMAGE then
        local screen_w = love.graphics.getWidth()
        local screen_h = love.graphics.getHeight()

        -- scaling relative to 1920x1080
        local _xscale = screen_w / 1920
        local _yscale = screen_h / 1080
        ...
        love.graphics.draw(
            ...
            draw_y,
            0,
            _xscale * scale_factor,
            _yscale * scale_factor
        )
    end
end

----------------------------------------------
------------FLOOR IS LAVA CODE END----------------------

----------------------------------------------
------------NO SWEAT INSURANCE CODE BEGIN----------------------
...
SMODS.Seal {
    ...
    loc_txt = {
        ...
        name  = 'Zero Point Seal',
        text = {
            "When {C:attention}drawn{} draw {C:attention}2{} additional cards",
            "Idea: BoiRowan"
        }
    },
    ...
    calculate = function(self, card, context)
        ...
        if not context.hand_drawn then return end

        -- Loop through drawn cards properly
        for i, drawn_card in ipairs(context.hand_drawn) do
...
SMODS.Back{
	...
	calculate = function(self, card, context)
        if context.final_scoring_step and G.GAME.current_round.hands_left == 0 and G.GAME.chips + SMODS.calculate_round_score() < G.GAME.blind.chips then
...
-- ==========================================
-- ZORLODO DECK DEFINITIONS
-- ==========================================

local ZORLODO_DECKS = {

    Blue = {
        text = "Blue Deck: +1 Hand",
        apply = function()
    ...
    Black = {
        text = "Black Deck: +1 Joker Slot, -1 Hand",
        apply = function()
            G.jokers.config.card_limit = G.jokers.config.card_limit + 1
            G.GAME.round_resets.hands = (G.GAME.round_resets.hands or 0) - 1
            ease_hands_played(-1)
    ...
    Magic = {
        text = "Magic Deck: +1 Consumable Slot, +2 Fools",
        apply = function()
            for _ = 1, 2 do
                local c = create_card('Tarot', G.consumeables, nil, nil, nil, nil, 'c_fool')
                c:add_to_deck()
                G.consumeables:emplace(c)
            end
            G.consumeables.config.card_limit = G.consumeables.config.card_limit + 1
    ...
    Ghost = {
        text = "Ghost Deck: +1 Hex, 2X Spectral Rate ",
        apply = function()
            local c = create_card('Tarot', G.consumeables, nil, nil, nil, nil, 'c_hex')
            c:add_to_deck()
            G.consumeables:emplace(c)
    ...
    Zodiac = {
        text = "Zodiac Deck: +1 Shop Slot, 2x Tarot/Planet rate",
        apply = function()
            change_shop_size(1)
    ...
    Painted = {
        text = "Painted Deck: +2 Hand Size, -1 Joker Slot",
        apply = function()
            G.hand.config.card_limit   = G.hand.config.card_limit + 2
            G.jokers.config.card_limit = G.jokers.config.card_limit - 1
        end,
    ...
    Anaglyph = {
        text = "Anaglyph Deck: Boss gives Double Tag",
        on_boss = function()
    ...
    Plasma = {
        text = "Plasma Deck: Balance Chips and Mult",
        on_score = function()
            return { balance = true }
        end
    },
    ...
    OG = {
        text = "OG Deck: +2 Joker Slots, No Vouchers",
        apply = function()
    ...
    ZB = {
        text = "Zero Build Deck: -2 Joker Slots, Vouchers Restock",
        apply = function()
            G.jokers.config.card_limit = G.jokers.config.card_limit - 2
        end,
        revert = function()
...
local function get_deck_pool()
    local t = {}
    for k in pairs(ZORLODO_DECKS) do
        t[#t+1] = k
    end
    return t
end

local function reroll_zorlodo(z)
    local pool = get_deck_pool()

    for i = #pool, 1, -1 do
        if pool[i] == z.deck then
            table.remove(pool, i)
        end
    end

    z.deck = pseudorandom_element(pool, pseudoseed('zorlodo' .. (G.GAME.seed or '')))
    z.triggered = false
    z.triggered2 = false

    show_zorlodo_effect(z.deck)
end
...
function show_zorlodo_effect(deck)
    local data = ZORLODO_DECKS[deck]
    ...
    G.E_MANAGER:add_event(Event({
        ...
        func = function()
            ...
            attention_text({
                ...
                text = data.text,
                maxw = 12,
                hold = G.SETTINGS.GAMESPEED * (#data.text * 0.04 + 1.2),
                align = 'cm',
                offset = { x = 0, y = -1 },
                major = G.play
            })
...
function Game:start_run(args)
    ...
    if G.GAME.zorlodo and G.GAME.zorlodo.deck then
        show_zorlodo_effect(G.GAME.zorlodo.deck)
    end
end
...
SMODS.Back{
    ...
    loc_vars = function()
        if not G.GAME.zorlodo then
            G.GAME.zorlodo = { deck = nil }
        end
        return { vars = { G.GAME.zorlodo.deck or "None" } }
    end,
    ...
    apply = function()
        ...
        G.GAME.zorlodo = {
            deck = pseudorandom_element(pool, pseudoseed('zorlodo')),
            triggered = false,
            triggered2 = false
        }
    end,
    ...
    calculate = function(_, _, context)
        ...
        local z = G.GAME.zorlodo
        if not z or not z.deck then return end

        local data = ZORLODO_DECKS[z.deck]
        if not data then return end

        if not z.triggered and data.apply then
            z.triggered = true
            data.apply()
        end
        ...
        if data.on_boss and context.setting_blind and G.GAME.blind.boss then
            data.on_boss()
        end

        if data.on_score and context.final_scoring_step then
            return data.on_score()
        end

        -- ZB Voucher Restock
        if z.deck == "ZB" and G.STATE == G.STATES.SHOP then
            if G.shop_vouchers and #G.shop_vouchers.cards <= 0
            and G.shop_booster and #G.shop_booster.cards > 0
            and not z.triggered2 then
...
do
    ...
    if type(_create_shop_card_ui) == 'function' then
        _G.create_shop_card_ui = function(card, ...)
            if is_og_deck_active() and (is_voucher_card(card) or is_blocked_pack(card)) or G.GAME.zorlodo and G.GAME.zorlodo.deck and G.GAME.zorlodo.deck == "OG" and (is_voucher_card(card) or is_blocked_pack(card)) then
...
do
    ...
    if type(_create_shop_card_ui) == 'function' then
        _G.create_shop_card_ui = function(card, ...)
            ...
            if is_zb_deck_active() and is_voucher_card(card) or G.GAME.zorlodo and G.GAME.zorlodo.deck and G.GAME.zorlodo.deck == "ZB" and is_voucher_card(card) then
...
SMODS.Back{
    ...
    loc_txt = {
        name = 'Fortnite Deck',
        text = {
            'Only {C:purple}Fortlatro{} content',
        },
    },
...
----------------------------------------------
------------FORTNITE DECK CODE END----------------------

----------------------------------------------
------------SHOP DECK CODE BEGIN----------------------

-- Hook into Game:update(dt) to save shop data once something is available
local original_game_update = Game.update
function Game:update(dt)
    original_game_update(self, dt)

    -- Only attempt if in SHOP state and not saved yet
    if G.STATE == G.STATES.SHOP and not G.GAME.SHOP_SAVED and G.GAME.selected_back and G.GAME.selected_back.effect and G.GAME.selected_back.effect.center and G.GAME.selected_back.effect.center.key == 'b_fn_ShopDeck'then
        ...
        if G.shop_booster and G.shop_booster.cards then
            for i, card in ipairs(G.shop_booster.cards) do
                boosters[i] = card.config.center.key
            end
        end
		...
		if G.shop_vouchers and G.shop_vouchers.cards then
            for i, card in ipairs(G.shop_vouchers.cards) do
                vouchers[i] = card.config.center.key
            end
        end
        ...
        if not G.GAME.SHOP_SAVED then
            ...
            -- Debug output
            --print("[Fortlatro] Shop saved")
            --print("Boosters:", table.concat(boosters, ", "))
            --print("Vouchers:", table.concat(vouchers, ", "))
            --print("Jokers:", table.concat(jokers, ", "))
        end
    end
	if G.GAME.SAVED_SHOP and G.STATE == G.STATES.BLIND_SELECT and not G.GAME.SHOP_SAVED then
		G.GAME.SHOP_SAVED = true
		G.GAME.SHOP_CLEARED = false
	end
		
end


-- Hook into shop reroll
local old_reroll_shop = G.FUNCS.reroll_shop
function G.FUNCS.reroll_shop(e)
    -- Call original reroll logic first
    old_reroll_shop(e)

    -- Only apply if this is the Shop Deck
    if G.GAME.selected_back
        and G.GAME.selected_back.effect
        and G.GAME.selected_back.effect.center
        and G.GAME.selected_back.effect.center.key == 'b_fn_ShopDeck'
    then
        ...
        ----------------------------
        if G.shop_booster and G.shop_booster.cards then
            local booster_num = G.GAME.starting_params.boosters_in_shop + (G.GAME.modifiers.extra_boosters or 0)

            -- Remove existing boosters
            for i = #G.shop_booster.cards, 1, -1 do
                local c = G.shop_booster:remove_card(G.shop_booster.cards[i])
                c:remove()
            end

            -- Recreate new boosters
...
SMODS.Back{
    ...
    loc_txt = {
        name = 'Shop Deck',
        text = {
            '{C:green}Shop{} never {C:attention}changes{}',
			'{C:green}Rerolls{} change the entire {C:green}Shop{}',
        },
    },
...
SMODS.Sticker {
  ...
  calculate = function(self, card, context)
    ...
    -- default to config value or 5 if missing
    extra.chips = extra.chips or self.config.extra.chips or 5
    extra.mult = extra.mult or self.config.extra.mult or 1
